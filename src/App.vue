<template>
  <v-app>
    <v-navigation-drawer v-model="drawer" app clipped floating temporary>
     <v-list>
      ...
      <v-toolbar-title class="align-center">
        <span class="title">TelifsizVideo</span>
      </v-toolbar-title>
      </v-row>
      <v-list-item>
    ...
    <v-app-bar app clipped-left>
      ...
      <v-icon class="mx-4">mdi-video</v-icon> 
      <v-toolbar-title class="align-center">
        <span class="title">TelifsizFoto</span>
      </v-toolbar-title>
      <v-spacer/>
         ...
         <v-card>
           <v-list>
             <v-list-item>
               ...
               <v-list-item-content>
                 <v-list-item-title>Barış Can Çifçi</v-list-item-title>
                 <v-list-item-subtitle>Bilgisayar Mühendisi</v-list-item-subtitle>
               </v-list-item-content>
             </v-list-item>
           </v-list>
           <v-divider/>
           ...
           <v-card-actions>
             <v-spacer/>
             <v-btn color="primary" text @click="menu=false">ÇIKIŞ YAP</v-btn>
           </v-card-actions>
         </v-card>
          </v-menu> 
      </div>
      ...
      <v-container>
        <v-row class="mb-4">
          <v-col sm="6" md="4" lg="3" xl="2" v-for="(item,index) in 200" :key="index">
            <v-card>
              <v-img height="200px" :src="`https://picsum.photos/500/300?image=${(index+1)}`"
              :lazy-src="`https://picsum.photos/10/6?image=${(index+1)}`"></v-img>
